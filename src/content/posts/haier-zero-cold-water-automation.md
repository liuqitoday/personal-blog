...
title: "按一下开关，海尔热水器就开始零冷水循环"
tags:
  - "智能家居"
description: "用小米中枢虚拟事件触发，配合 banto6/haier 控制零冷水单次循环：先重置状态，等火焰点燃后再播报完成。"
---

家里用的是海尔零冷水燃气热水器。零冷水这件事本身不复杂：让机器先把管道里的冷水循环加热，再出水就不会先凉一下。麻烦的是触发方式——我不想打开海尔 App，也不想走到机器前面按面板，只想在门口按一下开关，然后听到音箱报「热水循环完毕」。
...
触发则完全走小米：中枢网关里做一个虚拟事件 `haier_hot_water_turn_on`，蓝牙开关或场景绑定到这个事件，HA 监听后再去驱动热水器。
...
第 6 步不是业务逻辑。小米中枢在 HA 重连时会回放最后一次虚拟事件，不置空的话，重启 Home Assistant 就会再跑一遍热水循环。这件事我在[上一篇](/posts/home-assistant-automation-troubleshooting-guide/)写过，这里只保留最后那一下。
