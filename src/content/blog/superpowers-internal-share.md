...
从 `obra/superpowers` 官方仓库看，它的定位不是"一个提示词插件"，而是建立在可组合 skills 和初始指令之上的完整软件开发方法论。它支持的不只是 `Claude Code`，还包括 `Codex App`、`Codex CLI`、`Cursor`、`Gemini CLI`、`Kimi Code`、`OpenCode`、`Pi`、`Hermes Agent` 等十多个 harness。
...
**善用 subagent-driven-development 的两段式 review。** 它每个任务都派一个全新的 subagent（避免上下文互相污染），然后做两轮检查：先查"有没有按 spec 做到"，再查"代码质量行不行"。这是我认为整套流程里性价比最高的部分，比一次性让主 Agent 从头写到尾稳得多。

**动手前先隔离工作区。** `using-git-worktrees` 会在新分支上开一个独立目录，跑通初始化并确认测试基线干净。这样即使中途翻车，你当前的本地分支也是安全的。
...
这篇文章最早写于 2026 年 7 月初，那时它大概是 v6.1.x。之后 v6.2.0 和 v6.3.0 两个版本做了不少实质性的优化，有几条直接影响了上面的结论。

**v6.3.0（2026-08-12）**

- **brainstorming 会按任务规模自动降档。** 现在它会把请求分成 `spike`、`bounded`、`architectural` 三类：小任务跳过双文档那套仪式，大改动才走完整流程——但无论哪条路径，动手前都仍然要你先批准。这一点很关键，因为它把"任务小的时候别硬上完整流程"从使用者的经验判断，变成了工具内置的行为。
- **subagent-driven-development 更省也更稳。** 控制器不再因为计划里的非致命冲突而卡住空转（v6.3.0 记录过一个真实会话为此卡了将近 9 小时）；形状相同的小任务会合并成一次派发，明显降低开销；implementer 和 reviewer 不能再自己派 subagent（之前会产出重复 review）。
- **收尾更安全。** 清理 worktree 时，如果目录里还有未提交的文件，它会停下来把文件列出来问你，而不是直接 `--force` 销毁。
- 新增了 `Devin CLI`、`Hermes Agent`、`Grok Build CLI` 三个 harness。

**v6.2.0（2026-07-23）**

- **整个 skill 库做了一轮压缩。** 删掉了很多"推销式"文案——对已经调用这个 skill 的读者反复讲它的好处，其实是废话。
- **subagent-driven-development 的工作区改成按 plan 隔离。** 以前同一个工作区里跑第二个 plan 时，可能会把上一个 plan 的进度记录当成自己的（真实发生过，还不止一次）；现在按 plan 分目录，评审全部通过后自动清理。
- **`finishing-a-development-branch` 不再默认提供"丢弃分支"选项。** 把一个会销毁已完成、已通过测试的成果的操作，和"合并"并排放，本身就是个坏设计。

但真正让我觉得这套东西"值得认真看待"的，是 v6.2.0 压缩时发生的一件事。

那一轮删减里，有人把 TDD skill 里一段"为什么必须先写测试"的说服性文字删掉了。结果在抗压测试中，Agent 面对"先写实现、测试回头补"的压力时，坚持 test-first 的比例从 8/10 掉到了 5/10。最后这段内容没有硬删，而是被改写后放了回去。

这恰好印证了我前面说的：**skill 不是文档，是塑造 Agent 行为的代码**。改一句话都可能改变行为，所以改动要做对抗性测试和 eval。这也正是我觉得这套方法论值得团队借鉴的地方——它意味着"我们希望 Agent 怎么工作"这件事，是可以被工程化管理和持续验证的，而不是停留在个人习惯和口口相传里。
...
好在 v6.3.0 之后，"小任务管得太多"这个最大的抱怨已经被内置处理了：它会自己判断规模、自动降档。所以你不需要为了小活去开关它。
