...
```mermaid
flowchart TB
  O["Orchestrator<br/>编排 / 规划"]
  L["LLM<br/>大脑"] --> O
  T["Tools<br/>手脚"] --> O
  M["Memory<br/>记忆"] --> O
  H["Harness<br/>验收基线 / 执行边界 / 反馈信号 / 回退手段"] -. 外围支撑 .-> O
```
...
flowchart TB
  ...
  R --> F["Feedback<br/>test / metric / trace / audit"]
  F --> L
  H["Harness<br/>验收基线 / 权限边界 / 反馈信号 / 回退手段"] -.-> L
  H -.-> T
  H -.-> R
  H -.-> F
```
...
```mermaid
flowchart TB
  U["用户问题"] --> T1["Thought<br/>我需要先查看项目结构"]
  T1 --> A1["Action<br/>list_files({ path: &quot;src&quot; })"]
  A1 --> O1["Observation<br/>Main.java, ReActLoop.java, ..."]
  O1 --> T2["Thought<br/>已确认目录结构，接下来读取核心类"]
  T2 -. 下一轮继续 .-> A2["Action<br/>read_file(...)"]
```
...
```mermaid
flowchart TB
  A["第 1 层：常驻层<br/>身份、硬约束、禁止项<br/>短、稳定、每轮都需要"]
  B["第 2 层：按需加载层<br/>Skills、领域知识、长文档<br/>默认不注入，命中再读"]
  C["第 3 层：运行时注入层<br/>时间、渠道、用户偏好、当前任务状态<br/>动态变化，放在后面"]
  D["第 4 层：记忆层<br/>MEMORY.md、历史摘要、会话档案<br/>可检索，不默认全量展开"]
  E["第 5 层：系统层<br/>Hooks、Linter、权限、审计<br/>尽量不进 Prompt，直接在代码里执行"]
...
```mermaid
flowchart LR
  I["Initializer<br/>理解任务<br/>拆分功能清单<br/>生成状态文件"] --> S["feature-list.json<br/>progress.json<br/>task notes"]
  S --> C["Coding Agent<br/>选下一项<br/>修改代码<br/>跑测试<br/>更新状态"]
  C --> E["session 结束 / 中断"]
  E --> R["下次启动时从状态文件恢复"]
  R --> S
```
