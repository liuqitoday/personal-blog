...
2. **设备（Device）与接口（Interface）的区别**：
   - **设备**偏向底层物理或数据链路层，例如 `wan`、`wan.30`（VLAN 子设备）。
...
1. 进入“网络 → 接口”，查看当前 `WAN` 接口绑定的是哪个物理设备（例如是 `wan` 还是已经带 VLAN 的 `wan.41`）。如果你当前能正常上网，**千万不要随意更改当前的 WAN 接口本身**。
...
3. **基础设备**选择你的 WAN 物理网卡（例如 `wan`）。
4. **VLAN ID** 填入你的 IPTV VLAN（假设为 `30`）。
5. 保存后，你将得到一个名为 `wan.30` 的新设备。
